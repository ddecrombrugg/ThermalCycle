...
f = @(lambda) l_hv*M_CH4 + T_2*(lambda*(1+(y-2*x)/4)*M_O2*mean(janaf('O2',T_c2))+ lambda*(1+(y-2*x)/4)*3.76*M_N2*mean(janaf('N2',T_c2)))...
    ...
    + (1+(y-2*x)/4)*3.76*lambda*M_N2*mean(janaf('N2',T_c3)));
...
% +0.1 pour des valeurs parfaites ! 
c_3 = Q_comb/(T_3-T_2);
s_3 = c_3*log(t_3/t_2)+0.1;

e_3 = (h_3-h_1)-(T_0+273.15)*(s_3-s_1);
...
while abs(t_4-t_4_iter)>0.01
    ...
    t_4 = t_3*r^(-eta_PiT*(R_f*((t_4-273.15)-T_3)/(c_pa_moy4*(t_4-273.15)-c_pa_moy3*T_3)));
end

T_4 = t_4-273.15;

c_pa_moy34 = (c_pa_moy4*T_4-c_pa_moy3*T_3)/(T_4-T_3);
h_4 = h_3 + c_pa_moy34*(T_4-T_3);
s_4 = s_3 - (1-eta_PiT)/eta_PiT*c_pa_moy34*log(t_4/t_3);
e_4 = (h_4-h_1)-(T_0+273.15)*(s_4-s_1);

%%%%%%%%%%%%
% Massflow %
%%%%%%%%%%%%

m_a = P_e/((1-k_mec)*(m_fa*(h_3-h_4)-(h_2-h_1)));
m_f = m_fa*m_a;
m_c = m_a/m_ac;
...
e_c = hhv + 15*(c_pc + comp_f_O2/m_O2*mean(janaf('O2',T_ec))...
    ...
    +t_0*comp_f_H2O/m_H2O*(S_H2O+mean(janaf('H2O',T_ec))*log(t_0/273.15));
...
perte_mecen = k_mec*(m_f*(h_3-h_4)+m_a*(h_2-h_1));
perte_echen = m_f*(h_4-h_1); 
W_m = m_fa*(h_3-h_4)-(h_2-h_1);
...
perte_rotex = m_f*(e_3-e_4)-m_a*(e_2-e_1)-m_a*W_m;
perte_combex = m_c*e_c-(m_f*e_3-m_a*e_2);
...
eta_mec = 1-(perte_mecen/(P_e+perte_mecen));
eta_toten = eta_mec*eta_cyclen;
eta_cyclex = m_a*W_m/(m_f*e_3-m_a*e_2);
eta_rotex = m_a*W_m/(m_f*(e_3-e_4)-m_a*(e_2-e_1));
eta_combex = (m_f*e_3-m_a*e_2)/(m_c*e_c);

%%%%%%%%%%%%%%%%%%%%%%%%
% Allocation solutions %
%%%%%%%%%%%%%%%%%%%%%%%%

% A compléter... Juste long et pas envie de le faire mtnt...

% Pas sûr du calcul des pertes d'échappements
end
